...
For a given \emph{sample point} $r_0$ in the crystal, the ASE Flux $\Phi_{ASE}$ can
...
\begin{equation}
  ...
  ~g(\lambda)~G_{\vec{r}\rightarrow\vec{r}_0}
  ~dV d\lambda
\end{equation}
This can be estimated using Monte Carlo methods. Therefore, the equation is
rewritten as
\begin{equation}
...
path of a single ray as described in formula \ref{eq:gain}.
