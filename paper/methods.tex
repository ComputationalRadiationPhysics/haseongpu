...
The rays per prism $rays_x$ and importance $imp_x$ is calculated as follows:
\begin{align}
...
Using the distribution of $rays(p_x)$ leads to a 
the modified gain calculation:
\begin{align}
...
Then, N was increased in two steps by 10 for the simulation with uniform sampling.
...
Figure \ref{plot:repetitive} shows the sampling rate
...
$3*10^{10}$ rays less to calculate. Because reducing the number of rays always
...
In this section, the presented methods IS, AS and RS are compared in duration
and precision.
In Figure \ref{graphic:methods_compare}, the runtime to reach a
...
distribution on the gain medium by same runtime of 10s. Uniform and IS 
produce higher MSE values than AS and RS.
\begin{figure}[H]
  \centerline{
