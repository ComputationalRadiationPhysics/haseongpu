...
\begin{figure}[H]
  \centerline
  {\resizebox{0.5\textwidth}{!}{\includegraphics{graphics/kernel_detail2.png}}}
...
\end{figure}
   % Furthermore, rays are grouped together in such a way that they have a
   % similar origin, which improves performance through better utilisation of the
   % GPU L1 cache.
...
before every  $\Phi_{ASE}$ kernel (see Figure \ref{graphic:multigpu}) and it is used 
to figure out which areas are more important to the result. 
The result of the importance sampling is a schedule for a specific sample point, that
describes in which prism a ray starts.
...
\[\Phi_{ASE~diff} = |\Phi_{ASE~importance} - \Phi_{ASE~no~importance}|\] of the 
result with and without importance sampling was calculated with 
each $10^5$ rays per sample point (see Figure \ref{graphic:importance}). 
Then the number of rays per sample point was increased for the simulation
without importance sampling.
The more rays are simulated, the smaller is $\Phi_{ASE~diff}$, what 
means that the results get closer and therefore the simulation
with importance sampling reaches same results by less
...
\begin{figure}[H]
  \centerline
  {\resizebox{0.45\textwidth}{!}{\includegraphics{graphics/phiASE_difference.png}}}
...
For comparision it is shown the impact of importance sampling
on the MSE values (see Figure~\ref{plot:importance2}). 
Already importance sampling reduces the MSE values for
many sample points considerable.
\begin{figure}[H]
  \centerline{
    \resizebox{0.5\textwidth}{!}{\includegraphics{plot/mse_importance.png}}}
  \caption{histogram of MSE values with and without importance sampling}
  \label{plot:importance2}
\end{figure}
By using also adaptive sampling, reduces the MSE values to a specific MSE threshold.
...
\begin{figure}[H]
  ...
  \caption{histogram of MSE values with and without adaptive sampling}
  \label{plot:adaptive}
\end{figure}
