...
\[\Phi_{ASE\Delta} = |\Phi_{ASE~I}(M) - \Phi_{ASE~\bar{I}}(N)|\] of the 
result with ($I$) and without ($\bar{I}$) importance sampling was calculated with 
$N,M = 10^5$ rays per sample point (see Figure \ref{graphic:importance}). 
Then, N was increased in two steps by 10 for the simulation without importance sampling.
...
or restarting the simulation with the same number of rays and a different
