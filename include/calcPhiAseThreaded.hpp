...
{
    bar.reset();
    calcPhiAse(devBundle, experiment, compute, hostMesh, mesh, result, minSample_i, maxSample_i, runtime, bar);
    // threadIds.emplace_back(
    //     std::thread(
    //         [&experiment, &compute, &hostMesh, &mesh, &result, minSample_i, maxSample_i, &runtime, &bar,
    //         &devBundle]()
    //         {
    //             calcPhiAse(
    //                 devBundle,
