...
constexpr float calcPhiAse(
    DevBundle<T_Device, T_Exec>& devBundle,
    ...
    Args&&... args)
{
    ...
    dim3 blockDim(128);
    dim3 gridDim(200); //@TODO check wether we can not use a 3dim grid matching our mesh dimensions - or this should
                       // atleast depend on the number of mesh points
    ...
    float gainSumHost = 0.0f;
    float gainSumSquareHost = 0.0f;
    ...
    //
    // Memory allocations
    auto dNumberOfReflectionSlices = alpaka::onHost::alloc<unsigned>(devBundle.device, experiment.maxRaysPerSample);
    ...
    for(unsigned sampleIdx = minSampleIdx; sampleIdx <= maxSampleIdx; ++sampleIdx)
    {
        ...
        importanceSamplingPropagation(
            ...
            dPreImportance);
        alpaka::onHost::reduce(queue, devBundle.executor, 0.0, dPreImpotanceReductionBuf, std::plus{}, dPreImportance);
        alpaka::onHost::wait(queue);
        double hSumPhi = 0.0;
        ...
        while(mseTooHigh)
        {
            ...
            while(run++ < compute.maxRepetitions && mseTooHigh)
            {
                ...
                // Start Kernel
                alpaka::onHost::fill(queue, dGainSum, float{0}, Vec1D{1});
                alpaka::onHost::fill(queue, dGainSumSquare, float{0}, Vec1D{1});
                ...
                if(experiment.useReflections)
                {
                    queue.enqueue(
                        ...
                        alpaka::KernelBundle{
                            ...
                            mesh,
                            dIndicesOfPrisms.data(),
                            dNumberOfReflectionSlices.data(),
                            dImportance.data(),
                            hRaysPerSampleDump,
                            dGainSum.data(),
                            dGainSumSquare.data(),
                ...
                threadLocalStridingRNG += (frameSpec.getNumFrames().product() * frameSpec.getFrameExtents().product());
                alpaka::onHost::wait(queue);
                ...
                assert(!isnan(hgainSumView[0]));
                assert(!isnan(hgainSumSquareView[0]));
                assert(!isnan(mseTmp));
