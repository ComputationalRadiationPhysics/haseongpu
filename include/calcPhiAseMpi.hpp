    ...
    Result& result)
{
    ...
    int const base = totalSamples / size;
    int const rem = totalSamples % size;

    int const localBegin = rank * base + std::min(rank, rem);
    int const localCount = base + (rank < rem ? 1 : 0);
    int const localEnd = localBegin + localCount;

    float runtime = 0.0f;

    if(localCount > 0)
    {
        calcPhiAse(devBundle, experiment, compute, hostMesh, mesh, result, localBegin, localEnd, runtime);
    }
