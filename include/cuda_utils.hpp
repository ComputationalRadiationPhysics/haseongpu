...
static unsigned const MIN_COMPUTE_CAPABILITY_MAJOR = 2;
static unsigned const MIN_COMPUTE_CAPABILITY_MINOR = 0;
 ...
 **/
template<class T>
T* copyToDevice(std::vector<T> const& v)
{
    T* deviceV;
...
template<class T>
void copyToDevice(std::vector<T> const& v, T* deviceV)
{
    CUDA_CHECK_RETURN(cudaMemcpy(deviceV, &(v[0]), v.size() * sizeof(T), cudaMemcpyHostToDevice));
}

template<class T>
void copyToDevice(std::vector<T> const& v, T* deviceV, unsigned size)
{
    assert(size <= v.size());
...
template<class T>
void copyToDevice(T const a, T* deviceV)
{
    CUDA_CHECK_RETURN(cudaMemcpy(deviceV, &a, sizeof(T), cudaMemcpyHostToDevice));
}
 ...
 **/
template<class T>
void copyFromDevice(std::vector<T>& v, T const* deviceV)
{
    CUDA_CHECK_RETURN(cudaMemcpy(&(v[0]), deviceV, v.size() * sizeof(T), cudaMemcpyDeviceToHost));
}

template<class T>
T copyFromDevice(T const* deviceV)
{
    T a;
