...
public:
    ...
    //Konstruktor
    //inline Vec3d(double X=0.0f,double Y=0.0f, double Z=0.0f):_x(X),_y(Y),_z(Z){}

    Vec3d(double const &X, double const &Y, double const &Z) : _x(X), _y(Y), _z(Z) {
    }
    ...
    const Vec3d& operator=(Vec3d const &v) {
        _x = v.x();
        _y = v.y();
        _z = v.z();
        return *this;
    ...
    inline bool operator==(Vec3d const &v) {
    ...
    inline Vec3d operator-(Vec3d const &v) const {
        return Vec3d(_x - v.x(), _y - v.y(), _z - v.z());
    }
    ...
    inline Vec3d operator*(double const &s) const {
        return Vec3d(s*_x, s*_y, s * _z);
    }

    //Multiplikation mit Skalar

    inline Vec3d& operator*=(double const &d) {
        _x *= d;
        _y *= d;
        _z *= d;
        return *this;
    ...
    inline double operator*(Vec3d const &v) const {
    ...
    inline Vec3d operator^(Vec3d const &v) const {
        return Vec3d(_y * v.z() - _z * v.y(), _z * v.x() - _x * v.z(), _x * v.y() - _y * v.x());
    }

    // Output Stream

    friend std::ostream& operator<<(std::ostream &os, const Vec3d & v) // Static?!
    {
...
bool intersectionRayTriangle(Vec3d &intersectionPoint, //Schnittpunkt
        const Vec3d &rayOrigin, //Ursprung des Strahls
        const Vec3d &rayDirection, //Richtungsvektor des Strahls
        const Vec3d &p1, //1.Punkt des Dreiecks
        const Vec3d &p2, //2.Punkt des Dreiecks
        const Vec3d &p3, //3.Punkt des Dreiecks
        ...
        bool berechneSchnittp = 1)//soll Schnittpunkt berechnet werden (1) oder nicht (0)
{
    ...
    Vec3d p1_rayOrigin; //=rayOrigin-p1;
...
int main(int argc, char** argv) {
    ...
    //Strahl
    Vec3d rayDirection(1., 0.1, 0.); //rayDirection(1,0,0)
    Vec3d rayOrigin(-4., 0., -0.3); //rayOrigin(-4,0,0)
