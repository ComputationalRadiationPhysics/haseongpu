...
% Daniel Albach                                         2012-06-03
% 
% version for the calculations for Christoph at LMU
...
pump.I = 7e3; % Pump intensity in W/cm²
pump.T = 1.5e-3; % pump duration in s
pump.wavelength = 940e-9;% pump wavelength in m
pump.r = 1.0; % pump radius
...
mode.extr = 0; %no extraction by some laser
...
%    beta_crystal=zeros(mesh_z,1);
   ...
   pump.I = intensity*exp(-(sqrt(p(i_p,1)^2)/pump.r)^pump.exp)*exp(-(sqrt(p(i_p,2)^2)/pump.r)^pump.exp);
