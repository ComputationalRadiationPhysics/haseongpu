...
#ifdef _WIN32
        printBar(dout(V_PROGRESS | V_NOLABEL), static_cast<int>(percentage * length), length);
#else
        printWave(dout(V_PROGRESS | V_NOLABEL), tic, static_cast<int>(percentage * length), length);
#endif

        dout(V_PROGRESS | V_NOLABEL) << "] ";
        dout(V_PROGRESS | V_NOLABEL) << std::setfill(' ') << std::setw(3) << static_cast<int>(percentage * 100) << "%";
        dout(V_PROGRESS | V_NOLABEL) << " (" << std::setfill(' ') << std::setw(fillwidthPart) << currentPart << "/"
                                     << maxNTotal << ")";
        dout(V_PROGRESS | V_NOLABEL) << " after " << humanRepresentation(timeSpent);
        dout(V_PROGRESS | V_NOLABEL) << " (" << humanRepresentation(timeTotal) << " total, "
