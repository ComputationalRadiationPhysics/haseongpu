...
namespace hase::utils
{
        ...
        unsigned const nInterpolations)
    {
        ...
        assert(hase::internal::isStrictlySorted(x));
        ...
        double const x_min = x.at(0);
        double const x_max = x.at(x.size() - 1);
        ...
        {
            interpolated_x[i] = x_min + (static_cast<double>(i) * x_range) / static_cast<double>(nInterpolations - 1);
        }
