    ...
    for(unsigned level_i = 0; level_i < numberOfLevels; ++level_i)
    {
        for(unsigned point_i = 0; point_i < verticesPerLevel; ++point_i)
        {
            vtkFile << std::fixed << std::setprecision(6) << vertexCoordinates[point_i] << " "
                    << vertexCoordinates[point_i + verticesPerLevel] << " " << level_i * thicknessOfLevel << std::endl;
        }
    }
    ...
    if(filestream.is_open())
    {
        while(filestream.good())
        {
            ...
            if(foundLine)
            {
                ...
                if(diff >= smallDiff)
                {
                    dout(V_WARNING) << "ASE relative difference[" << ase_i << "]: " << diff << "%" << "["
                                    << compare.at(ase_i) << ", " << value << "]" << " > " << smallDiff << "%"
                                    << std::endl;
                }
