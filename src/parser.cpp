 ...
 *
 */
HostMesh createHostMeshFromFile(fs::path const rootPath)
{
...
bool validateHostMesh(HostMesh const& mesh)
{
    // assert input sizes
    ...
    assertRange(mesh.triangleNormalPoint, 0u, unsigned(mesh.numberOfPoints - 1), true);
    assertMin(mesh.betaVolume, 0, false);
    assertRange(mesh.forbiddenEdge, -1, 2, true);
    assertRange(mesh.triangleNeighbors, -1, int(mesh.numberOfTriangles - 1), true);
    assertRange(mesh.triangleNormalsX, -1, 1, false);
    assertRange(mesh.triangleNormalsY, -1, 1, false);
    ...
    assertRange(mesh.trianglePointIndices, 0u, unsigned(mesh.numberOfPoints - 1), true);
    assertMin(mesh.triangleSurfaces, 0, false);
...
void validateSampleRange(ComputeParameters& compute, unsigned const numberOfSamples)
{
    ...
    if(!minSet && !maxSet)
    {
        dout(V_INFO) << CompSwitch::min_sample_i << "/" << CompSwitch::max_sample_i
    ...
    if(minSet != maxSet)
    {
        throw std::runtime_error(
            "Options " + std::string(CompSwitch::min_sample_i) + "/" + std::string(CompSwitch::max_sample_i)
    ...
    {
        throw std::runtime_error(
            std::string(CompSwitch::max_sample_i) + " < " + std::string(CompSwitch::min_sample_i) + "!");
    }

    if(compute.minSampleRange >= numberOfSamples)
    {
        throw std::runtime_error(
            std::string(CompSwitch::min_sample_i) + " is out of range! (There are only "
    ...
    {
        throw std::runtime_error(
            std::string(CompSwitch::max_sample_i) + " is out of range! (There are only "
            + std::to_string(numberOfSamples) + " sampling points)");
    }
    ...
    dout(V_WARNING) << "Manual sampling range set via " << CompSwitch::min_sample_i << "/" << CompSwitch::max_sample_i
                    << ". This may allocate more memory than actually required. "
                       "Normally, the sampling range is determined automatically from the number of mesh points "
                       "and the number of layers, so setting it manually is not required for most use-cases."
                    << std::endl;
}
...
void validateInput(ExperimentParameters& experiment, ComputeParameters& compute)
{
    // if(compute.backend != Backend::CPU && compute.backend != Backend::GPU)
    // {
    //     throw std::runtime_error(
    //         CompSwitch::backend + " must be either \"" +
    //         std::string(Backend::CPU) + "\" or \"" +
    //         std::string(Backend::GPU) + "\"");
    // }

    if(compute.backend == Backend::CPU && compute.parallelMode != ParallelMode::SINGLE)
    {
        dout(V_WARNING) << CompSwitch::backend << " \"" << Backend::CPU << "\" does only support "
                        << CompSwitch::parallel_mode << " \"single\"! (will be ignored)" << std::endl;
    }
    ...
    {
        std::stringstream ss;
        ss << CompSwitch::parallel_mode << " must be one of \"" << ParallelMode::SINGLE << "\"";
#if !defined(DISABLE_MPI)
        ss << ", \"" << ParallelMode::MPI << "\"";
#endif
    ...
    ensureOrThrow(experiment.minRaysPerSample > 0, "Please specify a positive value for --" + ExpSwitch::min_rays);
...
void initializeResult(Result& result, unsigned const numberOfSamples)
{
    result = Result(
    ...
    Result& result)
{
    ...
    hostMesh.emplace(createHostMeshFromFile(vm[ExpSwitch::input_path].as<fs::path>()));
    hostMesh->calcTotalReflectionAngles();
    validateHostMesh(*hostMesh);
    bool const monochromatic = vm[ExpSwitch::monochromatic].as<bool>();
    ...
    compute = ComputeParameters(
        ...
        0,
        vm[CompSwitch::backend].as<std::string>(),
    ...
    Result& result)
{
    for(int i = 1; i < argc; ++i)
    {
        ...
        if(name == "-cfg" || name == "--cfg")
        {
            ...
            dout(V_INFO) << "Parsing simulation options using .cfg file '" << cfgPath.string()
