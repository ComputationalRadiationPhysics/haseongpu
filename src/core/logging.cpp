...
namespace hase::core
{
    namespace
    {
        int envRank(char const* name)
        {
            ...
            return end == value ? 0 : static_cast<int>(parsed);
        }

        bool isLauncherHeadRank()
        {
            int const rank = envRank("OMPI_COMM_WORLD_RANK") + envRank("PMI_RANK") + envRank("PMIX_RANK");
            return rank == 0;
        }
    } // namespace
    ...
    std::ostream& dout(unsigned activation_level)
    {
        ...
        if((activation_level & (V_WARNING | V_INFO | V_STAT | V_PROGRESS)) && !isLauncherHeadRank())
        {
            return dummy;
        }
