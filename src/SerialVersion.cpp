...
namespace
{
            ...
            for(int i_z = 0; i_z < static_cast<int>(ctx.nr_layers) - 1; ++i_z)
            {
                ...
                if(N_rays[prismIndex] > 0)
                {
                    importance[prismIndex]
                        = static_cast<double>(ctx.NumRays) * ctx.surface_new[i_t] / surf_tot / N_rays[prismIndex];
                }
                else
                {
    ...
    BaseVersionSerialContext createContext(ExperimentParameters const& experiment, HostMesh& mesh)
    {
    ...
    uint32_t maxSampleI)
{
    ...
    for(PointM const& sample_i : points)
    {
        ...
        int realNumRays = 0;
        calcImportance(ctx, sample_i, importance, N_rays, 1);
        ...
        for(unsigned triangle_i = 0; triangle_i < ctx.N_cells; ++triangle_i)
        {
            ...
            for(unsigned layer_i = 0; layer_i < ctx.nr_layers - 1; ++layer_i)
            {
                ...
                {
                    PointM randPoint = genRandPoint(ctx, t_1, t_2, t_3, layer_i);

                    double const gain = propagation(
                        ctx,
                        randPoint,
                        sample_i,
    ...
    for(PointM const& sample_i : points)
    {
        ...
        m_result.phiAse[sampleIndex] /= 4.0 * 3.14159;

        double const gain_local
            = ctx.N_tot * betaCells[sampleIndex] * (ctx.sigma_e + ctx.sigma_a) - (ctx.N_tot * ctx.sigma_a);

        m_result.dndtAse[sampleIndex] = gain_local * m_result.phiAse[sampleIndex] / hostCrystalFluorescence;
    }
...
{
    auto context = createContext(m_experiment, m_mesh);

    auto points = createPoints(context);

    this->mainLoop(context, points, m_result.dndtAse, m_mesh.betaCells, m_mesh.crystalTFluo, minSampleI, maxSampleI);
}
