...
For external provider builds, make sure the C++ ``openPMD::openPMD`` package
and Python ``openpmd_api`` module come from compatible openPMD-api
installations and support the runtime backend you choose. Use
``utils/check_openpmd_compatibility.py`` before installing when in doubt. The
preflight prints the selected backend, the backends confirmed by the active
...
* Description:
  ...
  * ``OFF``: use an externally installed openPMD-api C++ package found through
    ...
    workflow.
  * ``ON``: use the FetchContent source-build provider. HASEonGPU fetches the
    pinned openPMD-api provider and enables ADIOS2, ADIOS2 SST, and HDF5
    support for the CMake build.
