...
New users should start with the :doc:`Python Interface Guide <pythonInterface>`.  The legacy interface is
...
* Description:
  ...
  It provides the absorption and emission cross sections,
  ...
  section, :math:`\sigma_a` and :math:`\sigma_e`, via ``s_abs`` and
  ``s_ems``. The wavelength fields ``l_abs`` and ``l_ems`` are ignored in
  monochromatic mode.
...
* Field layouts:
  ...
  ``tfluo``
    Type: scalar.
    Element type: ``float``.
    Fluorescence lifetime :math:`\tau` of the crystal.
