...
using TestApis = std::decay_t<decltype(alpaka::onHost::allBackends(
...
namespace
{
    ...
    HostMesh constructDummyMesh(double betaVolume, float topReflectivity = 1.0f)
    {
        HostMesh mesh;
    ...
    struct PropagationKernel
    {
        ...
        ALPAKA_FN_ACC void operator()(auto const& acc, DeviceMeshView const mesh, auto result) const
        {
            for(auto [id] : alpaka::onAcc::makeIdxMap(
                    acc,
                    alpaka::onAcc::worker::threadsInGrid,
                    alpaka::IdxRange{Vec1D{propagationBatchSize}}))
            {
                unsigned level = 0u;
                unsigned triangle = 0u;
                double const zStart = 0.05 + 0.01 * static_cast<double>(id);
                Ray ray{{1.0 / 3.0, 1.0 / 3.0, zStart}, {0.0, 0.0, 0.4}, 0.4f};

                auto const offset = id * propagationResultWidth;
                result[offset] = propagateRay(ray, &level, &triangle, mesh, 0.0, 0.0);
    ...
    struct ReflectionKernel
    {
        ...
        ALPAKA_FN_ACC void operator()(auto const& acc, DeviceMeshView const mesh, auto result) const
        {
                    ...
                    alpaka::IdxRange{Vec1D{propagationBatchSize}}))
            {
                ...
                Point startPoint{1.0 / 3.0, 1.0 / 3.0, z};
                Point endPoint{1.0 / 3.0, 1.0 / 3.0, z};

                result[id] = propagateRayWithReflection(
                    startPoint,
                    endPoint,
                    1u,
                    TOP_REFLECTION,
    ...
    std::vector<double> runPropagationKernel(HostMesh& hostMesh, T_Device& device, T_Executor const& executor)
    {
        ...
        queue.enqueue(
            executor,
            alpaka::onHost::FrameSpec{Vec1D{4u}, Vec1D{8u}},
    ...
    template<typename T_Device, typename T_Executor>
    std::vector<double> runReflectionKernel(HostMesh& hostMesh, T_Device& device, T_Executor const& executor)
    {
        auto queue = device.makeQueue();
        auto deviceMesh = hostMesh.toDevice(device);
...
TEMPLATE_LIST_TEST_CASE(
    "propagateRayWithReflection responds to surface reflectivity",
    "",
    TestApis)
{
    auto cfg = TestType::makeDict();
    ...
    TestApis)
{
    ...
    auto deviceMesh = hostMesh.toDevice(device);
    DevBundle devBundle{device, exec};
    ...
    importanceSamplingPropagation(devBundle, 0u, reflectionSlices, deviceMesh.toView(), 0.0, 0.0, preImportance);
    ...
    unsigned const distributedRays = importanceSamplingDistribution(
        devBundle,
        reflectionSlices,
        deviceMesh.toView(),
        raysPerSample,
